...
def format_sample_pickapic(raw_sample: dict[str, Any]) -> dict[str, Any]:
    ...
    Image.open(io.BytesIO(raw_better_image)).convert('RGB')
    Image.open(io.BytesIO(raw_worse_image)).convert('RGB')
    raw_sample = {
        'input_text': prompt,
        'input_image': [],
    }
...
def preprocess(tokenizer, processor, formatted_sample: dict[str, Any]):
    ...
    if formatted_sample['images'] is not None:
        dict = processor(raw_text, formatted_sample['images'], return_tensors='pt').to(
...
def process_data(gpu, input_data, model_path, output_path, cache_dir):
    ...
    for piece in tqdm(input_data, desc=f'Processing on GPU {gpu}'):
        ...
        if (
            'pixel_values' in preprocessed_sample.keys()
            and preprocessed_sample['pixel_values'] is not None
        ):
            ...
            processed = model.pre_tokenization(input_ids=preprocessed_sample['input_ids'])
