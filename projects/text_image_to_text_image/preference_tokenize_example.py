...
def format_sample_cham(raw_sample: dict[str, Any]) -> dict[str, Any]:
    """Formatting input sample, and change the related keys according to the training dataset."""
...
def preprocess(tokenizer, processor, formatted_sample: dict[str, Any]):
    ...
    if formatted_sample['better_images'] is not None:
        better_dict = processor(
            raw_better, formatted_sample['better_images'], return_tensors='pt'
    ...
    if formatted_sample['worse_images'] is not None:
        worse_dict = processor(raw_worse, formatted_sample['worse_images'], return_tensors='pt').to(
    ...
    for piece in tqdm(input_data, desc=f'Processing on GPU {gpu}'):
        ...
        if preprocessed_sample['better_pixel_values'] is not None:
            better_piece = model.pre_tokenization(
                ...
                pixel_values=preprocessed_sample['better_pixel_values'].to(
                    device=model.device, dtype=torch.bfloat16
                ),
            )
        else:
            better_piece = model.pre_tokenization(input_ids=preprocessed_sample['better_input_ids'])
        ...
        if preprocessed_sample['worse_pixel_values'] is not None:
            worse_piece = model.pre_tokenization(
                ...
                pixel_values=preprocessed_sample['worse_pixel_values'].to(
                    device=model.device, dtype=torch.bfloat16
                ),
            )
        else:
            worse_piece = model.pre_tokenization(input_ids=preprocessed_sample['worse_input_ids'])
        ...
        if (
            updated_piece['better_input_ids'].shape[0] <= 4096
            and updated_piece['worse_input_ids'].shape[0] <= 4096
        ):
