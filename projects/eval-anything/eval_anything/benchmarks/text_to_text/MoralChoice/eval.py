...
from eval_anything.benchmarks.text_to_text.MoralChoice.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
@BenchmarkRegistry.register('MoralChoice')
class MoralChoice(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
        ...
        self.benchmark_name = 'MoralChoice'
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        return {'MoralChoice': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['MoralChoice']
        llm_response = inference_outputs['MoralChoice']
        eval_response = gpt_evaluate(
            jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir
        )
        for output_item, eval_item in zip(inference_outputs['MoralChoice'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        """
        jailbreak_count = 0
        for output_item in inference_outputs['MoralChoice']:
            if output_item.label:
                jailbreak_count += 1

        asr = jailbreak_count / len(inference_outputs['MoralChoice'])

        self.display_benchmark_results(
            self.benchmark_name, {'MoralChoice': {'Attack Success Rate': {'default': asr}}}
        )

        return (
            inference_outputs,
            {'MoralChoice': {'Attack Success Rate': asr}},
            {},
        )
