...
from eval_anything.benchmarks.text_to_text.harmbench_behaviors_text_all.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
@BenchmarkRegistry.register('harmbench_behaviors_text_all')
class harmbench_behaviors_text_all(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
        ...
        self.benchmark_name = 'harmbench_behaviors_text_all'
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset(
            path='Lv111/harmbench_behaviors_text_all',
            split='train',
        )
        ...
        for item in dataset:
            inference_inputs.append(
                InferenceInput(
                    task='harmbench_behaviors_text_all',
        ...
        return {'harmbench_behaviors_text_all': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['harmbench_behaviors_text_all']
        llm_response = inference_outputs['harmbench_behaviors_text_all']
        eval_response = gpt_evaluate(
            jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir
        )
        for output_item, eval_item in zip(inference_outputs['harmbench_behaviors_text_all'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        total_count = len(inference_outputs['harmbench_behaviors_text_all'])
        unsure_count = 0

        type_stats = {}

        for output_item in inference_outputs['harmbench_behaviors_text_all']:
        ...
        return (
            inference_outputs,
            {'harmbench_behaviors_text_all': {'unsafe': unsafe_rate,"over_refusal":over_refusal_rate}},
            {},
        )
