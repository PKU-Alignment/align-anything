...
from eval_anything.benchmarks.text_to_text.MIC.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
@BenchmarkRegistry.register('MIC')
class MIC(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['MIC']
        llm_response = inference_outputs['MIC']
        eval_response = gpt_evaluate(
            jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir
        )
        for output_item, eval_item in zip(inference_outputs['MIC'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        total_count = len(inference_outputs['MIC'])
        unsure_count = 0

        type_stats = {}

        for output_item in inference_outputs['MIC']:
        ...
        return (
            inference_outputs,
            {'MIC': {'unsafe': unsafe_rate,"over_refusal":over_refusal_rate}},
            {},
        )
