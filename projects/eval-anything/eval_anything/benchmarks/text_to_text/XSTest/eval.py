...
from eval_anything.benchmarks.text_to_text.xstest-v2-copy.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
@BenchmarkRegistry.register('xstest-v2-copy')
class xstest-v2-copy(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
        ...
        self.benchmark_name = 'xstest-v2-copy'
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        return {'xstest-v2-copy': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['xstest-v2-copy']
        llm_response = inference_outputs['xstest-v2-copy']
        eval_response = gpt_evaluate(
            jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir
        )
        for output_item, eval_item in zip(inference_outputs['xstest-v2-copy'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        total_count = len(inference_outputs['xstest-v2-copy'])
        unsure_count = 0

        type_stats = {}

        for output_item in inference_outputs['xstest-v2-copy']:
            question_type = output_item.metadata.get('type', 'unknown')
        ...
        return (
            inference_outputs,
            {'xstest-v2-copy': {'unsafe': unsafe_rate,"over_refusal":over_refusal_rate}},
            {},
        )
