...
class CMTrainer(CMtextTrainer):
    ...
    def eval_cm(self) -> dict[str, Any]:
        ...
        for batch in eval_dataloader:
            ...
            better_sign_list = batch['is_better_safe']
            worse_sign_list = batch['is_worse_safe']
            higher_end_rewards_list = higher_end_rewards.tolist()
            lower_end_rewards_list = lower_end_rewards.tolist()
            for item in higher_end_rewards_list:
                rewards_list.append(item)
            for item in lower_end_rewards_list:
                rewards_list.append(item)

            for item in better_sign_list:
                label_list.append(item)
            for item in worse_sign_list:
                label_list.append(item)
        ...
        rewards_list = torch.tensor(rewards_list).to(device)
        ...
        gathered_rewards_list = [torch.empty_like(rewards_list) for _ in range(dist.get_world_size())]
        gathered_label_list = [torch.empty_like(label_list) for _ in range(dist.get_world_size())]
        

        dist.all_gather(gathered_rewards_list, rewards_list)
        dist.all_gather(gathered_label_list, label_list)
        if is_main_process():
            rewards_list = torch.cat(gathered_rewards_list, dim=0)
            label_list = torch.cat(gathered_label_list, dim=0)
        print(len(rewards_list.tolist()))
        save_dict['rewards'] = rewards_list.tolist()
        save_dict['labels'] = label_list.tolist()
        import json
        if is_main_process():
            dir = "/aifs4su/chenxinyu/2024_9_13_cost_model_output/test_cm_90k_weight_8epoch_3.json"
