...
class PPOTrainer(RLTrainerBase):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, cfgs, ds_cfgs) -> None:
        ...
        if self.advantage_estimator in ["rloo", "reinforce_baseline", "group_norm"]:
            assert self.n_samples_per_prompt > 1, f"{self.advantage_estimator} requires n_samples_per_prompt > 1"
    ...
    def rollout(self, prompt_only_batch: PromptOnlyBatch) -> list[dict[str, Any]]:
        ...
        for i in range(0, total_batch_size, micro_batch_size):
            ...
            mini_batch = {
                k: v.repeat_interleave(self.n_samples_per_prompt, dim=0) 
                if isinstance(v, torch.Tensor) else 
                [item for item in v for _ in range(self.n_samples_per_prompt)]
            ...
            action_mask = actor_batch['attention_mask'][:, 1:].bool()
            ...
            micro_training_batch['action_mask'] = action_mask
    ...
    def get_advantages_and_returns(
        ...
        if self.advantage_estimator == 'gae':
            ...
            for t in reversed(range(start, seq_len)):
            ...
            if self.advantage_estimator == 'rloo':
                ...
                baseline = (rewards.sum(-1, keepdim=True) - rewards) / (self.n_samples_per_prompt - 1)
                rewards = rewards - baseline
                rewards = rewards.view(original_shape)
            elif self.advantage_estimator == 'reinforce_baseline':
                original_shape = rewards.shape
                rewards = rewards.reshape(-1, self.n_samples_per_prompt)
                ...
                std = rewards.std(-1, keepdim=True) + 1e-9
    ...
    def cumulative_returns(self, rewards, mask, start):
        
        response_length = rewards.size(1) - start
        ...
        # Mask invalid responses if mask is provided
        if mask is not None:
...
def main():
    ...
    task = os.path.join('text_to_text', 'multi_ppo')
