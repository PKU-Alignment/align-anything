...
class PPOTrainer(PPOTextTrainer):  # pylint: disable=too-many-instance-attributes
    ...
    def split_ptx_micro_batches(
        ...
        for i in range(0, total_batch_size, micro_batch_size):
            micro_batch = {}
            tmp_batch = ptx_batch
            if 'video_grid_thw' in tmp_batch:
                original_thw = tmp_batch.pop('video_grid_thw')
                original_pixel_values = tmp_batch.pop('pixel_values_videos')
                video_grid_thw = original_thw[i : i + micro_batch_size]
            ...
            if 'image_grid_thw' in tmp_batch:
                ...
                image_grid_thw = original_thw[i : i + micro_batch_size]
    ...
    def rollout(self, prompt_only_batch: PromptOnlyBatch) -> list[dict[str, Any]]:
        ...
        for i in range(0, total_batch_size, micro_batch_size):
            ...
            mini_batch = {}
            tmp_batch = prompt_only_batch

            if 'video_grid_thw' in tmp_batch:
                original_thw = tmp_batch.pop('video_grid_thw')
                original_pixel_values = tmp_batch.pop('pixel_values_videos')
                # video_grid_thw is a tensor of shape (n, 3), where we need to build a prefix tree in shape (n, 1), where the second dimension is the product of the three original elements
                video_grid_thw = original_thw[i : i + micro_batch_size]
                mini_batch['video_grid_thw'] = video_grid_thw
                # select video_pixel_values according to the video_grid_thw
                thw_products = torch.prod(original_thw, dim=1)
            ...
            if 'image_grid_thw' in tmp_batch:
                ...
                original_pixel_values = tmp_batch.pop('pixel_values')
                image_grid_thw = original_thw[i : i + micro_batch_size]
                mini_batch['image_grid_thw'] = image_grid_thw
                # select image_pixel_values according to the image_grid_thw
                thw_products = torch.prod(original_thw, dim=1)
    ...
    def rl_step(
        ...
        new_labels = inference_batch['input_ids'].clone()
        # change 151643 in the labels to -100
        new_labels[new_labels == 151643] = -100
        inference_batch['labels'] = new_labels
    ...
    def init_models(self) -> None:
        ...
        # loading actor model
        # print(f"Loading actor model from {self.cfgs.model_cfgs.actor_model_name_or_path}")
