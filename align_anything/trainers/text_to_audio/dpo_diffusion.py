...
class DPOTrainer(SupervisedTrainerBase):
    ...
    def init_models(self) -> None:
        ...
        self.ref_model, _, _, _, _ = load_pretrained_audio_diffusion_models(
            self.cfgs.model_cfgs.model_name_or_path,
            trust_remote_code=True,
            freeze_unet=True,
            dtype=self.dtype,
    ...
    def loss(self, batch: PreferenceBatch) -> dict[str, torch.Tensor]:
        ...
        prompt_embeds = self.text_encoder(batch['input_ids'], attention_mask=None)[0]
        prompt_embeds = torch.cat([prompt_embeds] * 2, dim=0)
        noisy_latents = self.noise_scheduler.add_noise(latents, noise, timesteps)
        model_pred = self.model(
            noisy_latents, timesteps, encoder_hidden_states=None, class_labels=prompt_embeds
        ).sample
