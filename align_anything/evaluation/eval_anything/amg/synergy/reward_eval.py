...
def reward_eval_ia(example):
    ...
    return sigmoid(model(**process_ia(prompt, image_path, audio_path)).end_scores.squeeze(dim=-1).item())

def reward_eval_ta(example):
    response = example["response"][0]
    audio_path = example["response"][1]
    prompt = example["prompt"]
    return sigmoid(model(**process_ta(prompt, response, audio_path)).scores.squeeze(dim=-1).mean().item())

def reward_eval_ti(example):
    response = example["response"][0]
    image_path = example["response"][1]
    prompt = example["prompt"]
    return sigmoid(model(**process_ti(prompt, response, image_path)).scores.squeeze(dim=-1).mean().item())
...
def main():
    ...
    with open(result_ia_file, "r") as f:
        result_ia = json.load(f)
    with open(result_ta_file, "r") as f:
        result_ta = json.load(f)
    with open(result_ti_file, "r") as f:
    ...
    with open("reward_score.json", "w") as f:
        json.dump({"reward_ia": np.mean(reward_ia), "reward_ta": np.mean(reward_ta), "reward_ti": np.mean(reward_ti)}, f)
