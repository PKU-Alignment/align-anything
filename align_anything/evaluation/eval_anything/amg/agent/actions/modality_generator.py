...
class AudioAudioGenPipeline(BaseDiffusionModelPipeline):
    ...
    def generate(self, prompts: str, instruction_uid) -> str:
        ...
        audio_write(save_path, audio[0].cpu(), self.pipe.sample_rate, strategy="loudness", loudness_compressor=True)
...
class ModalityGenerator(BaseAction):
    ...
    def generate(
        ...
        image_saved_path = self.image_pipeline.generate(image_prompt, instruction_uid) if image_prompt else None
        video_saved_path = self.video_pipeline.generate(video_prompt, instruction_uid) if video_prompt else None
        audio_saved_path = self.audio_pipeline.generate(audio_prompt, instruction_uid) if audio_prompt else None
