...
- `BENCHMARK`: One or more evaluation benchmarks for assessing the model's performance. For instance, if this parameter is set to `("POPE" "MMBench")`, the model can be evaluated using both the POPE and MMBench datasets. Each benchmark in the list will be processed sequentially.
...
- `BENCHMARKS`: One or more evaluation benchmarks for assessing the model's performance. For instance, if this parameter is set to `("POPE" "MMBench")`, the model can be evaluated using both the POPE and MMBench datasets. Each benchmark in the list will be processed sequentially.
- `OUTPUT_DIR`: The directory for saving the evaluation results and output files.
- `GENERATION_BACKEND`: The backend used for generation, `vLLM` or `deepspeed`.
- `MODEL_IDS`: An array containing two unique identifiers for the models being evaluated, such as `("llava-1.5-7b-hf" "llava-1.5-13b-hf")`. These IDs help track and distinguish between different model evaluations.
- `MODEL_NAME_OR_PATHS`: An array containing two local paths to the model weights or their Hugging Face links, such as `("llava-hf/llava-1.5-7b-hf" "llava-hf/llava-1.5-13b-hf")`.
- `CHAT_TEMPLATES`: An array containing two chat template IDs corresponding to each model, such as `("LLAVA" "LLAVA")`.
