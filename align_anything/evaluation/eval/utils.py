...
def create_openai_chat_completion(
    client: OpenAI, messages: List[dict], parameters: dict, key: str, retry_steps=0
) -> tuple[ChatCompletion | Exception, str]:
    try:
    ...
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        ...
        for id, key, input in zip(range(len(inputs)), full_keys, inputs):
            ...
            future = executor.submit(
                create_openai_chat_completion, client, input, parameters, key, 0
            )
        ...
        while len(futures) > 0:
            ...
            futures = new_futures
            print(
                'One round of futures done',
                len(futures),
                'with new futures to go',
                len(new_futures),
            )
