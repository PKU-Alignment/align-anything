...
def parse_eval_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--chat_template',
        default='',
        help='Chat template id of your model, details can be referred in `align-anything/align_anything/configs/eval_template.py`.',
    )
    parser.add_argument(
        '--benchmark',
        '-b',
...
def cli_evaluate(args: Union[argparse.Namespace, None] = None) -> None:
    ...
    if len(sys.argv) == 1:
        ...
        print(
            '│ More default configs can be referred in `align-anything/align_anything/configs`  │'
        )
        print('└─────────────────────────────────────────────────────────────────────────────────┘')
        sys.exit(1)
    ...
    try:
        ...
        if args.generation_backend == 'vllm':
            ...
            if 'eval.py' not in file_names:
                eval_logger.log(
                    'warning', 'Non-accelerating backend is not support for this benchmark.'
                )
                if 'vllm_eval.py' in file_names:
                    eval_logger.log('info', 'Generating responses using vLLM backend.')
                    args.generation_backend = 'vllm'
