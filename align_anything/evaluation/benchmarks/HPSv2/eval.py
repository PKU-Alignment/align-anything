    ...
    def parallel_eval(self, task2details, img_dir, data, device, position):
        ...
        for task, inputs in data.items():
            for prompt in tqdm(inputs, desc='Generating', position=position):
                ...
                task2details[task].extend([{'prompt': prompt, 'image_path': image_path}])
    ...
    def eval(self, data, img_dir):
        ...
        for i in range(num_processes):
            ...
            for task, inputs in data.items():
                ...
                chunks[task] = chunk
            p = mp.Process(
                target=self.parallel_eval, args=(task2details, img_dir, chunks, device, i)
            )
            p.start()
            processes.append(p)
        for p in processes:
