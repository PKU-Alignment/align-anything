...
class ChronoMagicBenchDataLoader(BaseDataLoader):
    ...
    def load_dataset(self) -> DatasetDict:
        ...
        for task in self.task_names:
            ...
            for data in dataset:
                processed_inputs[task].append({'prompt': data['name'], 'video_id': data['videoid']})
...
class ChronoMagicBenchGenerator(BaseInferencer):
    ...
    def parallel_eval(self, task2details, img_dir, all_data, device, position):
        ...
        for task, data in all_data.items():
            for input in tqdm(data, desc='Generating', position=position):
                ...
                task2details[task].extend(
                    [{'video_id': video_id, 'prompt': prompt, 'video_path': video_path}]
                )
    ...
    def eval(self, data, video_dir):
        ...
        for i in range(num_processes):
            ...
            for task, inputs in data.items():
                ...
                chunks[task] = chunk
            p = mp.Process(
                target=self.parallel_eval, args=(task2details, video_dir, chunks, device, i)
            )
            p.start()
            processes.append(p)
        for p in processes:
    ...
    def evaluator(self, outputs, task_dir, file_path):
        ...
        for output in tqdm(outputs, desc='Processing videos', unit='video', leave=False):
            ...
            video_data[video_id] = {'prompt': prompt, 'video_path': video_path, 'frames': frames}

        all_data = build_gpt_prompt(video_data, frames_dir)
        progress_bar = tqdm(total=len(video_data), desc='Evaluating')
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            future_idx = {
                executor.submit(self.save_output, prompt, file_path): vid
                for vid, prompt in all_data.items()
            }
...
def main():
    ...
    eval_module = ChronoMagicBenchGenerator(
