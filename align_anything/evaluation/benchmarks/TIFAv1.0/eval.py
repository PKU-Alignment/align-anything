...
class TIFADataLoader(BaseDataLoader):
    ...
    def load_dataset(self) -> DatasetDict:
        ...
        for task in self.task_names:
            ...
            processed_inputs[task] = [data['caption'] for data in datas]
...
def main():
    ...
    dataloader = TIFADataLoader(dict_configs)
    ...
    for task, outputs in raw_outputs.items():
        ...
        QA_pairs_path = f"{uuid_path}/QA_pairs.json"
        score, num_sum = eval_module.evaluator(outputs, file_path, QA_pairs_path, api_key, base_url)
        tot_score += score
        tot_num_sum += num_sum
        ...
        logger.print_table(title=f'TIFA v1.0/{task} Benchmark', data=eval_results)
        logger.log('info', '+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    ...
    logger.print_table(title=f'TIFA v1.0 Benchmark', data=eval_results)
    logger.log('info', '+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    logger.log('info', f"model_id: {eval_results['model_id'][0]},")
    logger.log('info', f"num_fewshot: {eval_results['num_fewshot'][0]},")
    logger.log('info', f"chain_of_thought: {eval_results['chain_of_thought'][0]},")
