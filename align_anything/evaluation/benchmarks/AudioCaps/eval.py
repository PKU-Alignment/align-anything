        ...
        for task in self.task_names:
            ...
            for data in dataset:
                ...
                assert os.path.exists(
                    audio_path_real
                ), f'Audio file {audio_path_real} does not exist.'
...
class AudioCapsGenerator(BaseInferencer):
    ...
    def eval(self, data, audio_dir):
        ...
        for i in range(num_processes):
            ...
            for task, inputs in data.items():
                ...
                chunks[task] = chunk
            p = mp.Process(
                target=self.parallel_eval, args=(task2details, audio_dir, chunks, device, i)
            )
            p.start()
            processes.append(p)
        for p in processes:
    ...
    def evaluator(self, outputs, file_path, score_path, background_dir, eval_dir):
        num_sum = 0
        os.system(
            f"python fad_score.py --datasetpath '{background_dir}' --logs '{eval_dir}' --name '{score_path}'"
        )
...
def main():
    ...
    eval_module = AudioCapsGenerator(
    ...
    for task, outputs in raw_outputs.items():
        ...
        score, num_sum = eval_module.evaluator(
            outputs, file_path, score_path, background_dir, eval_dir
        )
