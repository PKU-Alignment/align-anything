    ...
    def build_example_prompt(self, data, with_answer=True, cot=False):
        ...
        for label in data['choices']['label']:
            if ord('A') <= ord(label) <= ord('Z'):
                choice_id = label
                choices_text.append(
                    f'({choice_id}): {data["choices"]["text"][ord(choice_id) - 65]}'
                )
            else:
                choice_id = int(label)
...
def main():
    ...
    task_dirs = [
        (task, os.path.join(cache_path, task))
        for task in os.listdir(cache_path)
        if os.path.isdir(os.path.join(cache_path, task))
    ]
