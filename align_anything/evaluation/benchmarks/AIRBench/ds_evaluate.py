...
def evaluator(test_dataset, output_data, api_key, base_url, file_path):
    ...
    for output_item in tqdm(output_data, desc='Evaluating'):
        for test_item in test_dataset:
            if (
                test_item['uniq_id'] == output_item['question_id']
...
def judger(meta_info, question, answer_gt, response, api_key, base_url):
    for _ in range(5):
        ...
        prompt = (
            gpt_prompt.replace('XAudioX', meta_info)
...
def main():
    ...
    for task_name, _ in raw_outputs.items():
        test_data = load_dataset(
            data_cfgs.task_dir, split='train', data_files='Chat/Chat_meta.json'
        )
        file_path = f'{uuid_path}/{task_name}.json'
        num_score, num_sum = evaluator(
            test_data, raw_outputs[task_name], api_key, base_url, file_path
        )
    ...
    for task_id in task_ids:
        ...
        output_dict = {
            ...
            'average_score': [
                (
                    score_sum_dict[task_id] / total_num_dict[task_id]
                    if total_num_dict[task_id] > 0
                    else 0
                )
            ],
