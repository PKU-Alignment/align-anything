    ...
    def __call__(
        ...
        if raw_speech is not None:
            audio_inputs = self.audio_processor(
                raw_speech, sampling_rate=sampling_rate, return_tensors=return_tensors
            )
        else:
            audio_inputs = {}
        if text is not None:
...
class LlamaVisionAudioForConditionalGeneration(LlamaVisionAudioPreTrainedModel):
    ...
    def _merge_input_ids_with_audio_features(
        ...
        # - 1 to adjust for zero-based indexing, as `cumsum` inherently increases indices by one.
        new_token_positions = (
            torch.cumsum((special_audio_token_mask * (num_audio_patches - 1) + 1), -1) - 1
        )
        ...
        audio_to_overwrite &= audio_to_overwrite.cumsum(-1) - 1 >= nb_audio_pad[:, None].to(
            target_device
        )
        ...
        final_embedding[audio_to_overwrite] = (
            audio_features.contiguous().reshape(-1, embed_dim).to(target_device)
        )
    ...
    def forward(
        ...
        audio_feature_layer = (
            audio_feature_layer
            if audio_feature_layer is not None
            else self.config.audio_feature_layer
        )
        ...
        if inputs_embeds is None:
            ...
            if image_pixel_values is not None and input_ids.shape[1] != 1:
                ...
                image_outputs = self.vision_tower(image_pixel_values, output_hidden_states=True)
            ...
            if audio_pixel_values is not None and input_ids.shape[1] != 1:
                ...
                selected_audio_feature = selected_audio_feature.view(
                    (selected_audio_feature.size(0), selected_audio_feature.size(1), -1)
                ).permute(0, 2, 1)
