...
from align_anything.models.modeling_emu3.mllm.utils_emu3 import Emu3PrefixConstrainedLogitsHelper
...
class Emu3Processor(ProcessorMixin):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        """
        assert mode in ('G', 'U', 'TG', 'TU'), "mode must be 'G' or 'U' or 'TG' or 'TU'."
        ...
        if mode == 'G':
            ...
            image_outputs = image_outputs.to(
                self.vision_tokenizer.device, self.vision_tokenizer.dtype
            )
            ...
            raise ValueError("Invalid mode. Please specify 'G', 'U', 'TG', or 'TU'.")
        ...
        for idx, text_prompt in enumerate(text):
            ...
            if mode == 'U':
                ...
                h, w = self.calculate_generate_size(
                    ratio, image_area, self.vision_tokenizer.spatial_scale_factor
                )
                image_prompt = (
                    self.tokenizer.boi_token
                    + self.prefix_template.format(H=h, W=w)
                ...
                prompt += (
                    self.chat_template.format(image_prompt=image_prompt, text_prompt=text_prompt)
                    + output_text
                )
