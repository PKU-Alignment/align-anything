...
class RemoteRewardModel:
    ...
    def score(self, 
        ...
        """
        assert len(prompts) == len(responses), "The number of prompts and responses must be the same"
        ...
        for attempt in range(self.retry_times):
            try:
                ...
                print(f"Remote reward API request error (attempt {attempt+1}/{self.retry_times}): {e}")
                import time
                time.sleep(1)
