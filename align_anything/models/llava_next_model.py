...
class AccustomedLlavaNextModel(LlavaNextForConditionalGeneration):
    ...
    def forward(
        ...
        #     # 如果为 None，基于 pixel_values 的形状来推算
        #     batch = pixel_values.shape[0]  # 高度
        #     a = pixel_values.shape[2]  # 高度
        #     b = pixel_values.shape[3]  # 宽度
        #     image_sizes = [[a, b]] * batch  # 假设每个输入图像的大小相同
        
        if pixel_values is not None:
            batch = pixel_values.shape[0]  # 高度
            a = pixel_values.shape[2]  # 高度
            b = pixel_values.shape[3]  # 宽度
            # ori_image_num_patches = [a // batch] * batch
            ori_image_num_patches = []
            # image_sizes = [[a, b]] * batch
            image_sizes = [(a, b)] * batch
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None and input_ids.shape[1] != 1 and pixel_values.size(0) > 0:
                ...
                for patches in ori_image_num_patches:
                    ...
                    rec_pixel_value = pixel_values[start_idx:end_idx, :, :, :]
                    rec_pixel_values.append(rec_pixel_value)
                    start_idx = end_idx

                # Determine whether pixel_values is concatenated or stacked
                ...
                image_num_patches = [1] * image_features.size(0)
                
                image_features = torch.split(image_features, image_num_patches, dim=0)
                
                image_features = image_features[:input_ids.size(0)] 
...
def forward_n(
    ...
    cache_position: Optional[torch.LongTensor] = None,
    num_logits_to_keep: int = 0,
    task_specific_features: Optional[torch.Tensor] = None,  # 新增: 任务特定特征输入
    fusion_strategy: Optional[str] = "concat",             # 新增: 多模态融合策略
    debug_mode: bool = False,                              # 新增: 调试模式
) -> Union[Tuple, LlavaNextCausalLMOutputWithPast]:
    """
    Args:
        ...
        task_specific_features (`torch.Tensor`, optional):
            Task-specific features for auxiliary tasks or additional context.
        fusion_strategy (`str`, optional, default="concat"):
            Strategy to combine vision and text features ("concat", "add", "weighted").
        debug_mode (`bool`, optional, default=False):
            If True, logs intermediate outputs for debugging purposes.
    ...
    # 初始化 image_sizes
    if image_sizes is None and pixel_values is not None:
        batch = pixel_values.shape[0]
        a, b = pixel_values.shape[2], pixel_values.shape[3]
        image_sizes = [[a, b]] * batch
    ...
    if pixel_values is not None and inputs_embeds is not None:
        raise ValueError("Cannot specify both pixel_values and inputs_embeds")
    ...
    if image_features is not None:
        if fusion_strategy == "concat":
            ...
            inputs_embeds = alpha * inputs_embeds + (1 - alpha) * image_features
        else:
