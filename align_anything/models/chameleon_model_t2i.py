...
class ChameleonTextToImagePipeline:
    ...
    def build_response_from_segments(
        ...
        text_tokens_list = [token_ids for modality, token_ids in segments if modality == 'text']
        image_tokens_list = [
            token_ids[:1024] if len(token_ids) > 1024 else [1] * (1024 - len(token_ids)) + token_ids
        ...
        try:
            ...
            images = processor.postprocess_pixel_values(pixel_values.float().detach().cpu().numpy())
        except:
            images = None
    ...
    def generation(
        ...
        output_token_ids_batch = (
            output_token_ids_batch.to(dtype=inputs['input_ids'].dtype).detach().cpu().numpy()
        )
