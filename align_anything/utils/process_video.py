...
def get_video_processor(sample_frames: int, resolution: int, do_resize: bool=True, **kwargs) -> VideoProcessor:
    class CustomVideoProcessor(VideoProcessor):
        ...
        def extract_frames(self, frames: List[PIL.Image]) -> List[PIL.Image]:
            ...
            if num_frames >= 2 * self.sample_frames:
                selected_indices = np.linspace(0, 2 * self.sample_frames - 1, self.sample_frames, dtype=int)
        ...
        def __call__(self, video: Optional[Union[torch.Tensor, List[PIL.Image], List[List[PIL.Image]]]], height: Optional[int] = None, width: Optional[int] = None) -> torch.Tensor:
            if height == None:
