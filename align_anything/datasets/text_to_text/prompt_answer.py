...
def remove_duplicate_prompts(dict_list: list[dict[str, Any]], template):
    ...
    for idx in range(len(dict_list)):
        item = dict_list[idx]
        prompt = template.format_prompt_Answer_sample(item)[0]
...
class PromptAnswerSample(TypedDict, total=True):
    input_ids: torch.LongTensor  # size = (L,)
    labels: torch.LongTensor  # size = (L,)
    golden_answer: str


class PromptAnswerBatch(TypedDict, total=True):
    input_ids: torch.LongTensor  # size = (B, L)
    labels: torch.LongTensor  # size = (B, L)
    attention_mask: torch.BoolTensor  # size = (B, L)
    golden_answers: list[str]  # size = (B,)
...
class PromptAnswerDataset(Dataset):
    ...
    def preprocess(self, raw_sample: dict[str, Any]) -> PromptAnswerSample:
        return_dict = {}
        raw_text, golden_answer, _ = self.template.format_prompt_only_sample(raw_sample)
        if raw_text[-1] != self.tokenizer.eos_token:
            raw_text += self.tokenizer.eos_token
        return_dict['input_ids'] = self.tokenize(raw_text)
        return_dict['golden_answer'] = golden_answer

        return return_dict

    def get_collator(self) -> Callable[[list[dict[str, torch.Tensor]]], dict[str, torch.Tensor]]:
        return PromptAnswerCollator(self.tokenizer.pad_token_id)
...
class PromptAnswerCollator:
    ...
    def __call__(self, samples: list[PromptAnswerSample]) -> PromptAnswerBatch:
        return_dict = {}
        ...
        golden_answers = [sample['golden_answer'] for sample in samples]
        ...
        return_dict['golden_answers'] = golden_answers
