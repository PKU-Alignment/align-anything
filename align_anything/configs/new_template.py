...
class AA_TI2T:
    ...
    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        better_id = raw_sample['overall_response'] if raw_sample['overall_response'] in [1, 2] else 1
        answer = raw_sample[f'response_{better_id}']
        prompt = raw_sample['question']
        # prompt = raw_sample['prompt']
...
@register_template('LLAVA_revised')
class LLAVA_revised:
    system_prompt: str = ''
    user_prompt: str = 'USER: \n<image>{input}'
    assistant_prompt: str = '\nASSISTANT:{output}'
    ...
    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        raw_conversations = raw_sample
        raw_prompt = raw_conversations['question'].replace('<image>\n', '').replace('\n<image>', '')
        response = raw_conversations['revised_response']
        text = (
            f'{self.system_prompt}'
            f'{self.user_prompt.format(input=raw_prompt)}'
            f"{self.assistant_prompt.format(output=response)}"
        )
...
@register_template('SPA_VL_ours_test')
class SPA_VL_ours_test:
    ...
    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        if raw_sample["safer_response_id"]=="1":
        ...
        image = Image.open(image_path)
        if raw_sample["is_response_1_safe"]=="yes":
            raw_sample['is_response_1_safe'] = -1
        else:
            raw_sample['is_response_1_safe'] = 1
        if raw_sample["is_response_2_safe"]=="yes":
            raw_sample['is_response_2_safe'] = -1
        else:
            raw_sample['is_response_2_safe'] = 1
        if raw_sample["safer_response_id"]=="1":
            is_better_safe = raw_sample['is_response_1_safe']
            is_worse_safe = raw_sample['is_response_2_safe']
        else:
            is_better_safe = raw_sample['is_response_2_safe']
            is_worse_safe = raw_sample['is_response_1_safe']
    ...
    def check_equal(self, raw_sample: dict[str, Any]) -> bool:
        ...
        return raw_sample["more_helpful_response_id"] not in ["1","2"]
...
@register_template('moderation_model_origin')
class ModerationModelOriginTemplate:
    system_prompt: str = "A conversation between a user and an AI assistant. The assistant provides responses, and the task is to assess the safety of each response."
    user_prompt: str = 'USER: \n<image> {input}'
    assistant_prompt: str = '\nASSISTANT: {output}'
    split_token: str = 'ASSISTANT:'
    ...
    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        ...
        category = raw_sample.get("category", "default_category")
        label_map = {
            "terrorism_or_extremism": 0,
            "hacking_or_digital_crime": 1,
            "harmful_health_content": 2,
            "environmental_damage": 3,
            "dangerous_behavior": 4,
            "violence_and_physical_harm": 5,
            "psychological_harm_and_manipulation": 6,
            "animal_abuse": 7,
            "privacy_invasion_and_surveillance": 8,
            "identity_misuse_and_impersonation": 9,
            "discriminatory_depictions": 10,
            "insulting_and_harassing_behavior": 11,
            "deception_in_personal_relationships": 12,
            "financial_and_academic_fraud": 13,
            "false_information": 14,
            "sensitive_information_in_key_areas": 15,
            "horror_and_gore": 16,
            "psychological_horror_and_dark_themes": 17,
            "pornographic_content": 18,
            "sexual_crimes": 19
        }

        category_label = label_map.get(category, -1)
        formatted_prompt = f'{self.system_prompt}{self.user_prompt.format(input=prompt)}'
    ...
    def format_prompt_only_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        ...
        formatted_prompt = f'{self.system_prompt}{self.user_prompt.format(input=prompt)}{self.assistant_prompt.format(output="")}'
...
@register_template('moderation_model_old')
class ModerationModelOldTemplate:
    system_prompt: str = "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions.Remember that your answer should not contain image token."
    user_prompt: str = 'USER: \n<image> {input}'
    assistant_prompt: str = '\nASSISTANT: {output}'
    split_token: str = 'ASSISTANT:'

    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        if raw_sample["more_helpful_response_id"]=="1":
            better_response = raw_sample['response_1']
            worse_response = raw_sample['response_2']
        else:
            better_response = raw_sample['response_2']
...
@register_template('moderation_model')
class ModerationModelTemplate:
    system_prompt: str = "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions.Remember that your answer should not contain image token."
    system_prompt1: str = """
A conversation between a user and an AI assistant that processes multimodal input, including an image, a question, and a response. The assistant's task is to classify the input into one of the following categories based on the content of the image, the question, and the response:

Categories:
0. Terrorism or Extremism
1. Hacking or Digital Crime
2. Harmful Health Content
3. Environmental Damage
4. Dangerous Behavior
5. Violence and Physical Harm
6. Psychological Harm and Manipulation
7. Animal Abuse
8. Privacy Invasion and Surveillance
9. Identity Misuse and Impersonation
10. Discriminatory Depictions
11. Insulting and Harassing Behavior
12. Deception in Personal Relationships
13. Financial and Academic Fraud
14. False Information
15. Sensitive Information in Key Areas
16. Horror and Gore
17. Psychological Horror and Dark Themes
18. Pornographic Content
19. Sexual Crimes

For each input, the assistant should:
- Analyze the provided image for relevant features (e.g., objects, context, or visual cues).
- Consider the question for the context and specific inquiry.
- Evaluate the response for its relevance, tone, and content.

The assistant will classify the combination of the image, question, and response into one of these categories, returning the category index (0-19) based on the most relevant aspects of all three inputs.

The assistant should output only the integer index corresponding to the chosen category (0-19). 
If necessary, the assistant may provide a brief explanation of why this category was selected, but the main output should be the category index.
"""
...
@register_template('SPA_VL_ours_safe')
class SPA_VL_ours_safe:
    system_prompt: str = "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions."
    user_prompt: str = 'USER: \n<image> {input}'
    assistant_prompt: str = '\nASSISTANT: {output}'
    split_token: str = 'ASSISTANT:'
    ...
    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        if raw_sample["safer_response_id"]=="1":
            better_response = raw_sample['response_2']
            worse_response = raw_sample['response_1']
        if raw_sample["safer_response_id"]=="2":
            better_response = raw_sample['response_1']
            worse_response = raw_sample['response_2']
        # better_response = raw_sample['chosen']
        # worse_response = raw_sample['rejected']
    ...
    def check_equal(self, raw_sample: dict[str, Any]) -> bool:
        # return raw_sample['chosen'] == raw_sample['rejected']
        return raw_sample["safer_response_id"] not in ["1","2"]
...
@register_template('SPA_VL_ours_harmless')
class SPA_VL_ours_harmless:
    system_prompt: str = "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions."
    user_prompt: str = 'USER: \n<image> {input}'
    assistant_prompt: str = '\nASSISTANT: {output}'
    split_token: str = 'ASSISTANT:'
...
class LLAVA_RLHF:
    ...
    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        print(raw_sample)
        if raw_sample['preference'] == 1:
            better_response = raw_sample['output_1']['value']
            worse_response = raw_sample['output_2']['value']
        else:
            better_response = raw_sample['output_2']['value']
            worse_response = raw_sample['output_1']['value']
        if raw_sample['hallucination']:
            is_safe = 1
        else:
            is_safe = -1
            
        prompt = raw_sample['conversations'][0][9:]
        # image = raw_sample['images'][0]
        print("prompt: ", prompt)
        print("better:", better_response)
        print(is_safe)
        image_path = raw_sample['image']
        # read image from local path
        image = Image.open(image_path)   
        ...
        return {
            ...
            'worse_text': formatted_worse_output,
            'image': image,
            'is_better_safe': is_safe,
            # 'is_worse_safe': 0
        }

    def check_equal(self, raw_sample: dict[str, Any]) -> bool:
        return raw_sample['chosen']['value'] == raw_sample['rejected']['value']
...
class RLHF_V:
    ...
    def format_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        better_response = raw_sample['chosen']['value']
        worse_response = raw_sample['rejected']['value']
        prompt = raw_sample['conversations'][0]['value'][7:]
        image = raw_sample['images'][0]

        # image_path = raw_sample['images']
        #read image from local path
        # image = Image.open(image_path)   
        formatted_prompt = (
            f'{self.system_prompt}'
    ...
    def format_prompt_only_sample(self, raw_sample: dict[str, Any]) -> dict[str, Any]:
        prompt = raw_sample['conversations']['value'].replace('<image>\n', '').replace('\n<image>', '').replace('<image>', '')
        # image_path = raw_sample['images']
        # image = Image.open(image_path)
        image = raw_sample['images']
        formatted_prompt = (
